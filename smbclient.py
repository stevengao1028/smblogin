...
class Logon ():
    def __init__(self,root):
        ...
        self.box = ttk.Combobox(width=27)
        self.box.place(x=300,y=152)

        self.button2 = Button(root, bg="LightSteelBlue", text="手动加载", command=self.loadconf, width=29)
        self.button2.place(x=300, y=250)

        self.button3 = Button(root,bg="LightSteelBlue", text="删      　   除",command=self.delshare,width=29)
        ...
        with open(confile, 'r') as f1:
            for lines in f1.readlines():
                ...
                if len(line.split('##')) != 4:
                    continue
                confip = line.split('##')[2]
                confshare = line.split('##')[3]
                if ip == confip and share == confshare:
                    print share, confshare
                    self.lab1["text"] = "共享已经存在"
                    return 1
